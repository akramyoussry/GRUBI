...
class Quantum_Evo_Layer(layers.Layer):
    ...
    def compute_output_shape(self, input_shape):
        ...
        return (input_shape[0], input_shape[1], 2*self.num_wg)
###############################################################################
...
class complexMLmodel():
    ...
    def __init__(self, num_wg,  length):
        ...
        Parameters = tf.layers.Dense(num_wg**2, activation='linear')(Parameters)
        ...
        self.model.compile(optimizer=optimizers.RMSprop(lr=0.01), loss='mse', metrics=['mse'])
    ...
    def callibrate_zero_voltage(self, measurements):
        ...
        model.compile(optimizer=optimizers.RMSprop(lr=0.01), loss='mse', metrics=['mse'])
        model.summary()
