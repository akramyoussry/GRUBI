...
class Param_to_Ham_Layer(layers.Layer):
    ...
    def call(self, x):
        ...
        """    
        # add zero voltage parameters to the input, and reshape it to a column vector
        H = tf.expand_dims(x + self.beta0, -1) 
        ...
        # repeat the ioperator along the batch and time dimensions
        tri_op = tf.tile( self.tri_op, temp_shape )
        
        # construct a tensor in the form of a row vector whose elements are [d1,d2,n,n], where d1 and d2 correspond to the
        # number of examples and number of time steps of the input
        temp_shape = tf.concat( [tf.shape(x)[0:2],tf.constant(np.array([self.num_wg, self.num_wg],dtype=np.int32))],0 )

        #reshape into upper triangular matrix
        H = tf.reshape( tf.matmul(tri_op, H), temp_shape )
...
class MLmodel():
    ...
    def __init__(self, num_wg,  length):
        ...
        Parameters = layers.Dense(num_wg*(num_wg+1)//2, activation='linear')(Parameters)
        ...
        self.model.compile(optimizer=optimizers.RMSprop(), loss='mse')
    ...
    def construct_controller(self):
        ...
        # specify the optimizer and loss function for training
        controller_train_model.compile(optimizer=optimizers.RMSprop(), loss='mse')
