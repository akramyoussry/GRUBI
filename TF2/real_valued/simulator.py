...
class FourElectrodeArraySimulator(object):
    ...
    def __init__(self, num_wg, coupling_coeff, n0=2.1753, delta_n=5e-6,
        ...
        self.c0 = [c0 for _ in range(num_wg - 1)]
    ...
    def set_hamiltonian(self):
        ...
        h = np.zeros(shape=(self.num_wg, self.num_wg))
    ...
    def evolve(self, length, ideal=0):
        ...
        # calculate the probability amplitudes of the output states corresponding to normalized ouput power distribution
        self.output_distribution = np.array([float(abs(p) ** 2) for p in self.psi_out])
    ...
    def evolve_varying_voltage(self, time_range, input_voltage, length, ideal=0):
        ...
        wg_power         = np.zeros((time_range.size,self.num_wg)) 
