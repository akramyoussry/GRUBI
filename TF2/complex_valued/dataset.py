...
def create_dataset(filename, description, num_examples, rwa_simulator, time_range, seed=30, verbose=0):
    ...
    training_y       = [np.zeros((total_num_examples,num_points,2*num_wg)) for idx_wg in range(num_wg)] # output power distribution
    ...
    for idx_wg in range(num_wg):
        ...
        zero_voltage_measurements.append( np.reshape(rwa_simulator.evolve(rwa_simulator.length), (1,1,2*num_wg) ) )
